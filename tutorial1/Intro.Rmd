...
To end we show some code following the process for a basic data analysis. Namely
...
The first thing to do in R is to load data. For this tutorial we will use R to make up some data (using computer simulation). 

Our data will be two random normal vectors of x- and y-coordinates where y = x + error.
...
#4. We can print out the data so you can see it 
dummy
```
(Every time you run this, you'll get different numbers in your table because a new simulation is run).
...
summary(dummy__x)
```
```{markdown exsummary-hint}
We want to pass `dummy$x` to the function `summary()` 
```
**TASK FOR YOU** Calculate the mean of the first 10 values of `y`
```{r exsummary2, exercise=TRUE}
mean(dummy$__[__])
```
```{markdown exsummary2-hint}
Review the material in index vectors for what goes inside [__] 
```
...
We will next show some simple R plots (in time you might wish to use `ggplot2`, which has greater functionality but has a steeper learning curve). 
