...
The data is saved in a *data frame* that I named `dummy`. When you run your own analysis you will usually load data from a file instead (introduced in next tutorial).
...
```{r inspectdata, exercise=TRUE, exercise.setup="datagen"}
dummy
```

To get a quick summary of these data R has the useful function `summary()`

```{r sumdata, exercise=TRUE, exercise.setup="datagen"}
summary(dummy)
```
...
```{r onlyx, exercise=TRUE, exercise.setup="datagen"}
...
```{r exsummary, exercise=TRUE, exercise.setup="datagen"}
summary(dummy__x)
```
...
```{r exsummary2, exercise=TRUE, exercise.setup="datagen"}
mean(dummy$__[__])
```
...
```{r cortest, exercise=TRUE, exercise.setup="datagen"}
...
```{r spearman, exercise=TRUE, exercise.setup="datagen"}
...
```{r pearson-self, exercise=TRUE, exercise.setup="datagen"}
...
```{r plottest, exercise=TRUE, exercise.setup="datagen"}
...
```{r plottest2, exercise=TRUE, exercise.setup="datagen"}
...
For example, the following code follows this to write the above chart to a pdf file called `myplot.pdf`. 
```{r plottest3, exercise=TRUE, exercise.setup="datagen"}
pdf("myplot.pdf") #open up file writer for pdf
...
```{r plot-example, exercise=TRUE, exercise.setup="datagen"}
